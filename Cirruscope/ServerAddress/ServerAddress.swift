...
/// The type is pure and holds no reference to AppKit or Rainmaker, so `CirruscopeTests` can cover every rule without linking either. It is deliberately not `Equatable`: `inferredScheme` is a fact about the input rather than about the server, so two values naming the same instance would compare unequal purely because of how they were typed. Compare `url` where two addresses need to be told apart.
