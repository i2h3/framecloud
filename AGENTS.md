...
The project builds two apps from three source folders: `Cirruscope/` holds what both share, `macOS/` and `iOS/` hold what each needs alone. Each of the four targets carries its own `.xcconfig`, all of them chaining up to the root `Cirruscope.xcconfig`.
...
- `Cirruscope/` contains what the macOS and iOS app targets share, and is a member of both. `AppIcon.icon` is the app icon bundle (Icon Composer, gated to render on both platforms), `Assets.xcassets` holds the color assets, `Bundle+name.swift` reads `CFBundleName` at run time, `Cirruscope.entitlements` declares the App Group and Keychain-sharing capabilities, `Cirruscope.xcconfig` carries the bundle identity both apps share, and `PrivacyInfo.xcprivacy` is the privacy manifest.
- `macOS/` contains the Swift source code, resources, and configuration for the macOS app target, alongside its own `macOS.xcconfig`.
    ...
    - `ServerAddress/` contains `ServerAddressViewController` with its text-field-delegate and web-authentication extensions used to sign in, alongside the address-sanitation types the field relies on: `ServerAddress`, the pure value type that normalizes typed or pasted input into the one canonical address the app connects to and displays, `ServerAddressError`, the closed set of reasons an input cannot be one, and `ServerAddressFormatter`, the `Formatter` that rewrites the field to that canonical form as soon as editing in it ends.
...
- `iOS/` contains the Swift source code, resources, and configuration for the iOS app target, alongside its own `iOS.xcconfig`. The target exists ahead of any iOS release because a widget extension cannot be rendered in Xcode's Previews canvas from a macOS target, and because having it keeps the shared folder honest about what is actually platform-neutral. Its user interface is SwiftUI: `iOSApp.swift` is the `App` entry point, `ContentView.swift` its placeholder root, `Views/` holds the screens, and `Localizable.xcstrings` is the target's own String Catalog.
...
Localization lives in five String Catalogs (JSON, all with the same shape: each key maps to a `comment` plus a `localizations` dict of `<locale>: {"stringUnit": {"state": ..., "value": ...}}`), and all of them must stay complete for every detected localization:
...
for catalog in ["macOS/Localizable.xcstrings", "macOS/mul.lproj/Main.xcstrings", "macOS/InfoPlist.xcstrings", "macOS/AppShortcuts.xcstrings", "iOS/Localizable.xcstrings"]:
