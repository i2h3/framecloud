...
/// The messages are not asserted, only that each failure has one. Their text is localized, so asserting it would measure the language the machine running the tests is set to, and `CirruscopeTests` deliberately contributes no localized strings of its own.
